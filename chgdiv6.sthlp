...
{title:Syntax}
{p 4 4 2}
{cmdab:chgdiv6} [numbers]

{title:Description}
{p 4 4 2}{cmd:chgdiv6} 使用钱筮法,结果以卜辞形式呈现,钱筮法是大衍筮法的简化;{p_end}
{p 4 4 2}你可以在网络上查询这些卜辞的含义,希望能给你带来启示.{p_end}
{p 4 4 2}模式1-因时起卦:直接输入 {cmd:chgdiv6} 以当前的时间起卦,每一秒的结果都是不同的.{p_end}
{p 4 4 2}模式2-因数起卦:在 {cmd:chgdiv6} 后输入一串数字，会得到这串数字对应的结果,结果不因时间改变.{p_end}
...
{p 4 4 2}Mode 3 - Question / Character Divination: Enter your question after {cmd:chgdiv6}, and the program will extract a random number corresponding to the string. The result will not change over time.{p_end}
...
{p 4 4 2}{inp:.}{stata `"chgdiv6"'} {p_end}
{p 4 4 2}{inp:.}{stata `"chgdiv6 114514"'} {p_end}
